...
const balanceSchema = z.object({
  ...
  ships: z.object({
    ...
    purchases: z.array(z.object({ slug, name: z.string().min(1), description: z.string().min(1).optional(), credits: z.number().int().positive(), cargoCapacity: z.number().int().positive(), fuel: z.number().int().nonnegative(), visualKey: slug, successBonus: z.number().min(0).max(1).optional(), injuryReduction: z.number().min(0).max(1).optional(), lootRollBonus: z.number().int().nonnegative().optional(), maxCrew: z.number().int().positive().optional() })),
    ...
    upgrades: z.array(z.object({ slug, name: z.string().min(1), description: z.string().min(1), credits: z.number().int().nonnegative(), alloys: z.number().int().nonnegative().optional(), electronics: z.number().int().nonnegative().optional(), blueprints: z.number().int().nonnegative().optional(), conditionBonus: z.number().int().optional(), cargoBonus: z.number().int().optional(), fuelDiscount: z.number().int().optional(), repairDiscount: z.number().min(0).max(1).optional(), lootRollBonus: z.number().int().nonnegative().optional(), successBonus: z.number().min(0).max(1).optional() }))
  }),
  crew: z.object({ recruitCredits: z.number().int().nonnegative(), trainCreditsPerLevel: z.number().int().nonnegative(), maxRoster: z.number().int().positive(), injuryMinutes: range, shoreLeaveCredits: z.number().int().nonnegative(), fatiguePerExpedition: z.number().int().nonnegative(), candidateRotationHours: z.number().int().positive(), specialties: z.array(slug).min(1) }),
