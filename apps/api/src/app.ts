...
export async function buildApp() {
  ...
  await app.register(rateLimit, { max: env.RATE_LIMIT_MAX, timeWindow: `${env.RATE_LIMIT_WINDOW_SECONDS} seconds`, keyGenerator: request => request.ip });
  ...
  app.addHook('onRequest', async (request, reply) => {
    ...
    request.log.info({ requestId: request.id, method: request.method, route: request.routeOptions.url ?? 'unmatched' }, 'request started');
  });
  app.addHook('onResponse', async (request, reply) => {
    const startedAt = (request as typeof request & MetricsRequest).metricsStartedAt ?? performance.now();
    const route = request.routeOptions.url || 'unmatched';
    context.metrics.record(request.method, route, reply.statusCode, performance.now() - startedAt, Number(reply.getHeader('content-length') ?? 0));
  });
  app.setErrorHandler((error, request, reply) => {
    const response = errorResponse(error, isProd);
    request.log.error({ err: error, requestId: request.id, method: request.method, route: request.routeOptions.url ?? 'unmatched' }, 'request failed');
    if ('details' in response && response.details?.retryAfterSeconds) reply.header('retry-after', response.details.retryAfterSeconds);
    reply.code(response.statusCode).send({ error: { code: response.code, message: response.message, ...('details' in response ? { details: response.details } : {}) }, requestId: request.id });
  });
  app.addHook('onClose', async () => { await gameQueue.close(); await prisma.$disconnect(); });
