  ...
  return (
    <Panel depth="medium" className="station-schematic">
      ...
      <div className="station-orbit" aria-label="Station Zero operational schematic">
        <img className="station-orbit__art" src="/station/station-zero.webp" alt="Station Zero modular orbital station" />
        ...
        {modules.map((module, index) => {
          ...
          return <div key={module.slug} className={`station-node is-${module.state}`} style={{ '--node-angle': `${angle}deg`, '--node-radius': index % 2 ? '38%' : '45%' } as CSSProperties}><span><img src={`/station/modules/${module.slug}.webp`} alt="" /><b>{module.name}</b><i>{module.state}</i></span></div>;
        })}
      </div>
...
function WreckPanel({ wreck }: { wreck: Wreck | null }) {
  ...
  return (
    <Panel tone={tone} className="wreck-panel">
      ...
      <div className="wreck-scan">{wreck ? <img src={`/wrecks/${wreck.archetype}.webp`} alt={`${wreck.name}, current salvage target`} /> : <NWIcon name="wreck" size={70} />}<span /><span /><span /><strong className="nw-numeric">{wreck?.integrity ?? 0}% HULL</strong></div>
